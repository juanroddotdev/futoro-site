  ...
  <div ref="containerRef" class="container" :id="sectionId" :class="position">
    <div ref="phoneContainerRef">
      <FloatingPhone 
        ...
        :tilt-y="tiltY"
      >
        <div v-if="ambientMode" class="ambient-screen">
...
import { applyAmbientPullEffect, resetAmbientScreen } from '@/utils/ambientEffects';
...
const emit = defineEmits(['update:ambientMode']);
...
if (self.progress > 0.5 && props.ambientMode) {
  ...
  console.log('Progress:', progress.toFixed(3), 
              'MaxReached:', isMaxPullReached.value, 
              'Rebounding:', isRebounding.value);
  
  // Detect when we reach max pull (very close to 1.0)
  if (progress >= 0.995 && !isMaxPullReached.value && !isRebounding.value) {
    console.log('MAX PULL REACHED - TRIGGERING SPRING/BOUNCE:', progress.toFixed(3));
    ...
    // Reset after a delay to allow for future animations
    setTimeout(() => {
  ...
  // Store current progress for reference
