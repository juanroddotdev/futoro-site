...
/**
 * Animates dots in a U-shaped wave pattern
 * @param dots - NodeList or array of dot elements to animate
 * @param onComplete - Optional callback function to execute when animation completes
 * @returns GSAP timeline for the animation
 */
export function animateDotsUShapeEffect(
  dots: NodeListOf<Element> | HTMLElement[],
  onComplete?: () => void
): gsap.core.Timeline | undefined {
  if (!dots || dots.length === 0) {
    console.warn('[AmbientEffects] ⚠️ No dots found for U-shape animation');
    if (onComplete) onComplete();
    return;
  }
  
  console.log('[AmbientEffects] 🎬 Starting U-shape dots animation');
  
  // Create master timeline
  const masterTimeline = gsap.timeline({
    onComplete: () => {
      console.log('[AmbientEffects] ✅ U-shape animation complete');
      if (onComplete) onComplete();
    }
  });
  
  // Animate each dot in a U-shape pattern
  dots.forEach((dot, index) => {
    ...
    // Add this dot's timeline to the master timeline
    masterTimeline.add(timeline, 0); // All start at the same time, but with internal delays
  });
